...
long def_open( char *stream_name, long buffer_size, long *stream_size )
/*----------------------------------------------------------------------------
*/
{
...
long def_read( long handle, void *buffer, long num_bytes )
/*---------------------------------------------------------------
*/
{
   long read_size = -1;
...
#ifdef AMIGA // #7
   if( handle ) {
      read_size = Read( (BPTR)handle, buffer, num_bytes );
   }
#else
   if( handle ) {
...
int def_seek( long handle, long abs_byte_seek_pos )
/*--------------------------------------------------------
*/
{
   int err = 0;
...
//         CallHookPkt( &bitstream->baccess, (APTR)bitstream->file_handle, &param );
           CallHookPktPPC(&bitstream->baccess,(APTR)bitstream->file_handle,&param);
...
//      bs->file_handle = CallHookPkt( &bs->baccess, NULL, &param );
        bs->file_handle = CallHookPktPPC(&bs->baccess,NULL,&param);
...
//      bitstream->remain_bytes = CallHookPkt( &bitstream->baccess, (APTR)bitstream->file_handle, &param );
        bitstream->remain_bytes = CallHookPktPPC(&bitstream->baccess,(APTR)bitstream->file_handle,&param);
...
//      err = (int)CallHookPkt( &bitstream->baccess, (APTR)bitstream->file_handle, &param );
        err = (int)CallHookPktPPC(&bitstream->baccess,(APTR)bitstream->file_handle,&param);
