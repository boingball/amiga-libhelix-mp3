...
/*------------------------------------------------------------------------------
    ...
    19  |08/09/1999| Added update_info                                    ST
    20  |09/09/1999| Fix synchronize function to keep first frame         ST
    21  |12/09/1999| Added VBR (Xing) support                             ST
    22  |14/10/1999| Fixed SYNC_VALID bug (thanks Peter)                  ST
...
static const INT32 fr_bytes[ 4 ][ 2 ] = { /* #21 [ layer ][ id ] */
   { 0, 0 },
   { 48000, 48000 },
   { 144000, 144000 },
   { 72000, 144000 }
...
static int decode_header( MPA_STREAM *mps )
/*--------------------------------------------------------------------------
...
static int read_header( MPA_STREAM *mps, BOOL check )
/*--------------------------------------------------------------------------
...
*/
{
   ...
   // #22 Corrected freq index of 3 forbidden
...
//                (mps->sfreq == sfreq) && (mps->bitrate == bitrate) ) {
               ...
               err = BSTR_seek( mps->bitstream, hpos ); // #20
//               err = BSTR_seek( mps->bitstream, mph->header_pos ); // #20
               if( err ) return MPEGDEC_ERR_BADFRAME;
               err = read_header( mps, TRUE );
               return err;
//               return MPEGDEC_ERR_NONE;
               // #17 End
            }
...
static INT32 get_int32( BITSTREAM *bs ) {

   INT32 value;

   value = BSTR_read_byte( bs ) << 24;
   value |= BSTR_read_byte( bs ) << 16;
   value |= BSTR_read_byte( bs ) << 8;
   value |= BSTR_read_byte( bs );
   return value;
}

static void check_vbr( MPEGDEC_STREAM *mpds ) {
/*--------------------------------------------------------------------------
   Check the current MPEG Audio stream for vbr tag info
*/
   static const UINT8 vbr_tagid[] = { 'X', 'i', 'n', 'g' };
   MPA_STREAM *mps;
   int status;
   ...
   mps->vbr_flags = 0;
   mps->vbr_frames = mps->vbr_bytes = 0;
   mps->vbr_scale = -1;

   (void)BSTR_seek( mps->bitstream, mps->bitstream_start_pos );
   status = read_header( mps, FALSE );
   if( status == MPEGDEC_ERR_NONE ) { // Header found
      int i, offset;
      if( mps->header.ID == MPA_ID_1 ) {
         offset = ( mps->header.mode == MPA_MODE_MONO ) ? 17 : 32;
      }
      else {
         offset = ( mps->header.mode == MPA_MODE_MONO ) ? 9 : 17;
      }
      while( offset-- ) (void)BSTR_read_byte( mps->bitstream );
      // Check for VBR tagid
      for( i=0; i<4; i++ ) if( BSTR_read_byte( mps->bitstream ) != vbr_tagid[ i ] ) break;
      if( i == 4 ) {
         mps->vbr_flags = get_int32( mps->bitstream );
         if( mps->vbr_flags & MPEGAUD_VBR_FFRAMES ) {
            mps->vbr_frames = get_int32( mps->bitstream );
         }
         if( mps->vbr_flags & MPEGAUD_VBR_FBYTES ) {
            mps->vbr_bytes = get_int32( mps->bitstream );
         }
         if( mps->vbr_flags & MPEGAUD_VBR_FTOC ) {
            BSTR_read_bytes( mps->bitstream, MPEGAUD_VBR_TOCSIZE, mps->vbr_toc );
         }
         if( mps->vbr_flags & MPEGAUD_VBR_FSCALE ) {
            mps->vbr_scale = get_int32( mps->bitstream );
         }
      }
   }
}
...
static void update_info( MPEGDEC_STREAM *mpds )
/*--------------------------------------------------------------------------
   Update the current MPEG Audio stream information
*/
{
   MPA_STREAM *mps;
...
*/
{
   ...
   /* #21 Begin */
   mpds->ms_duration = 0;
   if( (mps->vbr_frames > 0) && (mps->sfreq > 0) ) {
      if( mps->vbr_bytes <= 0 ) mps->vbr_bytes = mps->stream_size;
      mpds->ms_duration = ((REAL)mps->vbr_frames * (REAL)(fr_bytes[ mps->header.layer ][ mps->header.ID ] << 3)) / (REAL)mps->sfreq;
//      mpds->bitrate = (mps->sfreq * mps->vbr_bytes) / (mps->vbr_frames * fr_bytes[ mps->header.layer ][ mps->header.ID ] );
      if( mpds->ms_duration >= 8 ) mpds->bitrate = mps->vbr_bytes / (mpds->ms_duration >> 3);
   }
   else {
...
*/
{
   ...
   if( mps->need_sync ) { // #10
      ...
      err = read_header( mps, FALSE ); // #5 FALSE: don't fail if MPEG Audio sync not at first
      if( err ) return err;
   }
...
*/
{
   ...
   if( mps ) {
      ...
      if( mps->vbr_flags & MPEGAUD_VBR_FTOC ) { // #21
         INT16 index;
         REAL fa, fb, fx;
         REAL percent = (REAL)(ms_time_position * 100) / (REAL)mpds->ms_duration;
         if( percent < 0.0 ) percent = 0.0;
         if( percent > 100.0 ) percent = 100.0;

         index = (INT16)percent;
         if( index > 99 ) index = 99;
         fa = (REAL)mps->vbr_toc[ index ];
         if( index < 99 ) fb = (REAL)mps->vbr_toc[ index + 1 ];
         else fb = 256.0;
         fx = (fa + (fb - fa ) * (percent - (REAL)index)) / 256.0;
         seek_pos = (INT32)(fx * (REAL)mps->vbr_bytes);
         frame = (INT32)( percent * 0.01 * (REAL)mps->vbr_frames );
      }
      else {
         exact_seek = ((REAL)mps->bitrate * (REAL)ms_time_position) * 0.125;
         // Calculate nearest # of frames
         frame_bytes = fr_bytes[ mps->header.layer ][ mps->header.ID ]; // #21
/*
...
*/
{
   ...
   if( mps ) {
      INT32 frame_bits = fr_bytes[ mps->header.layer ][ mps->header.ID ] << 3; // #21
/*
...
// #17 Begin
   ...
   check_vbr( mpds ); // #21
#else
   if( read_header( mps, (BOOL)ctrl->check_mpeg ) ) { // #6: check_mpeg
      (void)MPEGDEC_close( mpds );
      return NULL;
   }
#endif
