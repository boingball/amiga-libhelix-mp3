        ...
        if (x < 4) {
            ...
            if (x < 16) {
                ...
                */
                ...
                /*
                 * FIX: saturating left shift — prevent signed 32-bit overflow for
                 * large spectral coefficients (x >= 64).  The original unconditional
                 * "<< 3" wraps large positive values to large negative, corrupting
                 * the dequantized magnitude and producing white-noise on bass/beats.
                 */
                y = MULSHIFT32(y, pow2frac[shift]);
                if (y > (0x7fffffff >> 3))
                    y = 0x7fffffff;
                else
                    y <<= 3;
