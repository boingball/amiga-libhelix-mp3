...
#if defined(AMIGA_M68K)
    /* bsdsocket.library is per-task.  The GUI spawns a fresh playback process
     * for every Play, so a SocketBase opened by one (now-exited) child must not
     * leak into the next child, which would then make socket calls on a base
     * owned by a dead task and crash (Guru 0x80000008).  Close it here so each
     * child opens and closes its own base.  (The CLI is one process, so it
     * never hit this.) */
